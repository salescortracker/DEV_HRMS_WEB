...
// ✅ ONLY FIRST CLOCK IN (NEVER CHANGE)
this.firstClockIn = clockIns.length ? clockIns[0].actionTime : null;

// ✅ ONLY LAST CLOCK OUT (UPDATES ALWAYS)
this.lastClockOut = clockOuts.length
  ? clockOuts[clockOuts.length - 1].actionTime
  : null;

// 👉 UI DISPLAY RULE
this.todayClockIn = this.firstClockIn ?? '--:--';
this.todayClockOut = this.lastClockOut ?? '--:--';
this.clockInDisplay = this.firstClockIn ?? '--:--';
  ...
  // =========================
  // 🟡 STILL CLOCKED IN (LIVE)
  // =========================
  if (lastIn) {
    this.isClockedIn = true;
    this.clockStatus = 'Clocked In';

    this.clockInTime = lastIn;
    this.startTimer(totalMs, lastIn);
